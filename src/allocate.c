///////////////////////////////////////////////////////////////////////////////////////////////////
// Emerge code - File allocate.c                                                                 //
// These functions have been adapted from the GADGET code developed by Volker Springel           //
///////////////////////////////////////////////////////////////////////////////////////////////////
///
/// \file allocate.c
/// \brief Contains functions that allocate the memory using a memory manager
///
/// This module handles the dynamic memory allocation.
/// To avoid memory allocation/dellocation overhead a big chunk of memory (which will be the
/// maximum amount of dynamically allocatable memory) is allocated upon initialization. This
/// chunk is then filled by the memory blocks as in a stack structure. The blocks are
/// automatically aligned to a 64 bit boundary. Memory blocks come in two flavours: movable and
/// non-movable. In non-movable blocks the starting address is fixed once the block is allocated
/// and cannot be changed. Due to the stack structure of the dynamic memory, this implies that
/// the last (non-movable) block allocated must be the first block to be deallocated. If this
/// condition is not met, an abort condition is triggered. If more flexibility is needed, movable
/// memory blocks can be used. In this case, the starting address of the block is again fixed
/// upon allocation but the block can be shifted (therefore its initial address changes)
/// according to needs. For a movable block to be successfully shifted it is required that all
/// the subsequent allocated blocks are movable. Again, an abort condition is triggered if this
/// condition is not met. Movable blocks can be deallocated in any order provided that the
/// condition just described holds. The gap resulting form the deallocation of a block that is
/// not in the last position will be automatically filled by shifting all the blocks coming after
...
///////////////////////////////////////////////////////////////////////////////////////////////////
// Manager for dynamic memory allocation developed as part of the GADGET CODE by Volker Springel //
///////////////////////////////////////////////////////////////////////////////////////////////////

#define MAXBLOCKS 5000                     ///< Maximum number of blocks that can be allocated
#define MAXCHARS  19                       ///< Maximum number of characters a tag can have
...
void malloc_init(void)
{
	...
	if(n & 63)
		endrun("Want 64 byte aligned address");

	if(!(Base = malloc(n)))
	{
		printf("Failed to allocate memory for `Base' (%d Mbytes).\n", All.MaxMemSize);
		endrun("failure to allocate memory");
	}
...
void report_detailed_memory_usage_of_largest_task(size_t * OldHighMarkBytes, const char *label, const char *func, const char *file, int line)
{
	...
	sizelist = (size_t *) emalloc("sizelist", NTask * sizeof(size_t));
	MPI_Allgather(&AllocatedBytes, sizeof(size_t), MPI_BYTE, sizelist, sizeof(size_t), MPI_BYTE, MPI_COMM_WORLD);
	...
	if(maxsize > 1.1 * (*OldHighMarkBytes))
	{
		*OldHighMarkBytes = maxsize;
		...
		if(ThisTask == task)
		{
			char *buf = emalloc("buf", 200 * (Nblocks + 10));
			int cc = 0;
			cc += sprintf
			        (buf + cc, "\n\nAt '%s', %s()/%s/%d: Largest Allocation = %g Mbyte (on task=%d), Smallest = %g Mbyte, Average = %g Mbyte (Past Largest: %g Mbyte)\n\n",
			        label, func, file, line, maxsize / (1024.0 * 1024.0), task, minsize / (1024.0 * 1024.0), avgsize / (1024.0 * 1024.0),
			        GlobHighMarkBytes / (1024.0 * 1024.0));
			cc += dump_memory_table_buffer(buf + cc);
			if(task == 0)
			{
				fprintf(FpMemory, "%s", buf);
				fflush(FpMemory);
			}
			else
			{
				MPI_Send(&cc, 1, MPI_INT, 0, TAG_N, MPI_COMM_WORLD);
				MPI_Send(buf, cc + 1, MPI_BYTE, 0, TAG_HDATA, MPI_COMM_WORLD);
			}
			efree(buf);
		}
		...
		if(ThisTask == 0 && task > 0)
		{
			...
			char *buf = emalloc("buf", cc + 1);
			MPI_Recv(buf, cc + 1, MPI_BYTE, task, TAG_HDATA, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
			fprintf(FpMemory, "%s", buf);
			fflush(FpMemory);
			efree(buf);
		}
...
int dump_memory_table_buffer(char *p)
{
	...
	cc += sprintf(p + cc, "-------------------------- Allocated Memory Blocks----------------------------------------\n");
...
void *malloc_fullinfo(const char *varname, size_t n, const char *func, const char *file, int line)
{
	...
	{
		sprintf(msg, "Task=%d: No blocks left in malloc_fullinfo() at %s()/%s/line %d. MAXBLOCKS=%d\n", ThisTask, func, file, line, MAXBLOCKS);
		endrun(msg);
	}
...
void free_movable_fullinfo(void *p, const char *func, const char *file, int line)
{
	...
	{
		dump_memory_table();
		sprintf(msg, "Task=%d: Wrong call of free_movable() from %s()/%s/line %d - this block has not been allocated!\n", ThisTask, func, file, line);
		endrun(msg);
	}
	...
	if(nr < Nblocks - 1)    // the block is not the last allocated block
	{
		...
		for(i = nr + 1; i < Nblocks; i++)
			if(MovableFlag[i] == 0)
			{
				dump_memory_table();
				sprintf
				  (msg, "Task=%d: Wrong call of free_movable() from %s()/%s/line %d - behind block=%d there are subsequent non-movable allocated blocks\n",
...
void *realloc_fullinfo(void *p, size_t n, const char *func, const char *file, int line)
{
	...
	if(n > FreeBytes)
	{
		dump_memory_table();
		sprintf
		  (msg, "Task=%d: Not enough memory in remalloc(n=%g MB) at %s()/%s/line %d. previous=%g FreeBytes=%g MB\n",
...
void *realloc_movable_fullinfo(void *p, size_t n, const char *func, const char *file, int line)
{
	...
	{
		dump_memory_table();
		sprintf(msg, "Task=%d: Wrong call of realloc_movable() from %s()/%s/line %d - this block has not been allocated!\n", ThisTask, func, file, line);
		endrun(msg);
	}
	...
	if(nr < Nblocks - 1)    // the block is not the last allocated block
	{
		...
		for(i = nr + 1; i < Nblocks; i++)
			if(MovableFlag[i] == 0)
			{
				dump_memory_table();
				sprintf
				  (msg, "Task=%d: Wrong call of realloc_movable() from %s()/%s/line %d - behind block=%d there are subsequent non-movable allocated blocks\n",
				  ThisTask, func, file, line, nr);
				endrun(msg)
			}
	}
	...
	if(n > FreeBytes)
	{
		dump_memory_table();
		sprintf
		  (msg, "Task=%d: at %s()/%s/line %d: Not enough memory in remalloc_movable(n=%g MB). previous=%g FreeBytes=%g MB\n",
